...
| `Application.GetSystemVariable / SetSystemVariable` | **Shipped** — `{"op":"sysvar","get":[…]}` / `{"op":"sysvar","set":{…}}` over the readable registry (`ltscale`, `clayer`, `ctextstyle`, `celtscale`, `textsize`, `filletrad`, `mirrtext`, `insunits`, `osmode`, `pdmode`, `pdsize`, `extmin`, `extmax`); the writable subset applies atomically and `clayer`/`ctextstyle` validate against the tables. `state` also reports `hand_seed` (`Database.Handseed` parity, hex). |
| File identity (`SPMDOCUNIQUE`, Catalog §1.6) | **Shipped** — `{"op":"file_identity"}` (REST `POST /file-identity`): idempotent get-or-create of a stable RFC 4122 GUID, persisted on an invisible marker text via the XData application `SPMDOCUNIQUE` exactly like SPM.ACAD, so it survives save/open; `"renew":true` rewrites it. |
...
| `BlockTableRecord` + `AppendEntity` into a block (Catalog §2.3) | **Shipped** — `block_define` `{"name":"…","base":[x,y,z],"handles":[…]}` creates a definition from existing entities and optionally places one `Insert`; result returns definition + insert handles. Barcode re-import (CF-01.3) is covered by `"replace":true` (drops the same-named definition with its children and inserts in one undo step) and `block_delete` removes a definition completely. |
